# Docker镜像打包推送脚本：
1. **容错处理**：单个 Registry 推送失败不影响其他 Registry
2. **灵活标签**：支持 `-d dev.Dockerfile`（无后缀）和 `-d dev:dev.Dockerfile`（带后缀）两种格式
3. **错误不终止**：推送失败仅告警，继续执行后续流程
...
### 1. 容错处理（单个 Registry 失败不影响其他）
- 移除 `set -e`（脚本不再因单个命令失败而终止）
- 所有推送操作添加**返回值判断**，失败仅输出警告，继续执行后续流程
- 新增 `FAILED_REGISTRIES` 数组记录失败的推送任务，最后汇总输出
- 清理标签时失败也仅警告，不终止

### 2. 灵活的 Dockerfile 格式（支持带/不带后缀）
#### 格式1：带后缀（原逻辑）
```bash
./build.sh -d dev:dev.Dockerfile -b ubuntu -t 22.04
# 生成标签：tekintian/ubuntu:22.04-dev
```

#### 格式2：无后缀（新支持）
```bash
./build.sh -d dev.Dockerfile -b ubuntu -t 22.04
# 生成标签：tekintian/ubuntu:22.04
```

#### 标签适配逻辑：
- 带后缀：`tekintian/ubuntu:22.04-dev` → GHCR: `ghcr.io/tekintian/ubuntu:22.04-dev` → 阿里云: `registry.cn-hangzhou.aliyuncs.com/tekintian/ai:ubuntu_22.04-dev`
- 无后缀：`tekintian/ubuntu:22.04` → GHCR: `ghcr.io/tekintian/ubuntu:22.04` → 阿里云: `registry.cn-hangzhou.aliyuncs.com/tekintian/ai:ubuntu_22.04`

### 3. 增强的结果汇总
脚本最后会输出：
- ✅ 构建成功的所有镜像
- ❌ 推送失败的 Registry/镜像（如有）
- 明确提示“部分失败但已完成所有可执行操作”

### 4. 测试示例
#### 示例1：带后缀 + 多 Registry（模拟 GHCR 失败）
```bash
./build.sh \
  -b ubuntu \
  -t 22.04 \
  -d dev:dev.Dockerfile \
  -r ghcr \
  -r aliyun
```
执行效果：
- 构建：`tekintian/ubuntu:22.04-dev`
- 推送 Docker Hub（成功）
- 推送 GHCR（失败，仅警告）
- 推送阿里云（成功）
- 清理临时标签
...
```
生成标签：
- Docker Hub: `tekintian/ubuntu:22.04`
- 阿里云: `registry.cn-hangzhou.aliyuncs.com/tekintian/ai:ubuntu_22.04`

### 总结
1. **容错能力**：单个 Registry 推送失败不中断脚本，保障其他 Registry 正常推送。
2. **灵活标签**：支持带/不带后缀两种 Dockerfile 格式，满足不同镜像标签需求。
3. **结果可视化**：汇总输出成功/失败任务，便于排查问题。
4. **兼容性**：完全保留原有功能，仅新增容错和灵活标签特性。
